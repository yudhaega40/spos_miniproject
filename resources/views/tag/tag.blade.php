            ...
            @if(count($tag) === 0)
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-4">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        <p class="font-normal text-lg"> Tag tidak ditemukan </p>
                    </div>
                </div>
            @else
                    ...
                    <table class="w-full lg:table-fixed border-collapse border border-slate-400 mb-4 break-words">
                        <thead>
                            <tr class="bg-slate-300">
                                <th class="border border-slate-400 text-center py-2 lg:w-4/12">Nama Tag</th>
                        ...
                        <tbody>
                            @foreach ($tag as $t)
                            <tr>
                                <td class="border border-slate-400 text-center py-2"><a href='/post_by_tag/{{$t->id}}' class="font-semibold hover:text-blue-900">{{ $t->name }}</a></td>
                                <td class="border border-slate-400 text-center py-2">{{ $t->desc }}</td>
